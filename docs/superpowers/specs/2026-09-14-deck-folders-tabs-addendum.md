...
| Where new terminals land | **Last click wins:** `selectedFolderId` is the placement target. Clicking a folder header selects that folder; opening/activating a terminal (row or tab click) selects that terminal's folder. New terminals land in `selectedFolderId` (null = Unfiled). The selected section is highlighted, so the highlight always shows where ⌘T will put the next terminal. *(Amended 2026-09-14 after final review: the original "focused terminal's folder, else last-clicked header" rule made header clicks dead whenever a tab was open.)* |
